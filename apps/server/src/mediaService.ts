...
export const mediaPrepareSchema = z.object({
  ...
  if (value.purpose === 'chat' && !value.conversationId) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['conversationId'], message: 'CHAT_CONVERSATION_REQUIRED' });
  }
  if (value.purpose !== 'chat' && value.conversationId) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['conversationId'], message: 'CONVERSATION_NOT_ALLOWED' });
  }
});
...
type MediaRow = {
  ...
  mime_type: z.infer<typeof mimeSchema>;
...
async function presignObject(
  ...
  const response = await fetchImpl(url, {
    ...
    body: JSON.stringify({
      ...
      expires_in_seconds: operation === 'upload' ? 300 : 60,
    }),
...
export async function prepareMediaUpload(
  ...
  // Fail before creating any database row when provider signing is unavailable.
  ...
  await query(
    `insert into public.media_objects
      (id, owner_id, object_key, purpose, conversation_id, mime_type, byte_size, status)
     values ($1,$2,$3,$4,$5,$6,$7,'pending')`,
    [mediaId, userId, objectKey, request.purpose, request.conversationId ?? null, request.mimeType, request.byteSize],
  );
  ...
  try {
    ...
    // A failed signing attempt must never leave a client-usable pending object.
...
export async function completeMediaUpload(
  ...
  const result = await query<MediaRow>(
    `select id, owner_id, object_key, purpose, conversation_id, mime_type, byte_size, status
  ...
  const probe = await fetchImpl(signed.url, {
    method: 'GET',
    headers: { ...signed.headers, range: 'bytes=0-0' },
    signal: AbortSignal.timeout(10_000),
  });
  ...
  if (actualBytes == null || !Number.isSafeInteger(actualBytes) || actualBytes <= 0 || actualBytes > MAX_MEDIA_BYTES) {
    throw new Error('MEDIA_SIZE_UNVERIFIED');
  }
  ...
  const result = await query<MediaRow>(
    `select m.id, m.owner_id, m.object_key, m.purpose, m.conversation_id, m.mime_type, m.byte_size, m.status
      ...
      where m.id=$1 and m.status='ready'
        and (
          m.owner_id=$2
          or (m.purpose='chat' and m.conversation_id is not null and public.is_conversation_member(m.conversation_id,$2::uuid))
        )
      limit 1`,
    [mediaId, userId],
  );
...
export const KSSENGER_MEDIA_PROVIDER_SCOPE = Object.freeze({
  projectId: KSSENGER_NEON_PROJECT_ID,
  branchId: KSSENGER_NEON_BRANCH_ID,
  bucket: KSSENGER_MEDIA_BUCKET,
