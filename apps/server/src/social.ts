...
export async function requestContact(userId: string, recipientId: string) {
  ...
  const { data, error } = await supabaseAdmin.from('contact_requests').insert({ sender_id: userId, recipient_id: recipientId }).select('id,status').single();
  if (error) throw error;
  return data;
}
...
export async function acceptContact(userId: string, requestId: string) {
  ...
  const { error: updateError } = await supabaseAdmin.from('contact_requests').update({ status: 'accepted' }).eq('id', requestId);
...
export async function blockUser(userId: string, blockedId: string) {
  ...
  const { error } = await supabaseAdmin.from('blocks').upsert({ blocker_id: userId, blocked_id: blockedId });
  if (error) throw error;
  await supabaseAdmin.from('contacts').delete().or(`and(owner_id.eq.${userId},contact_id.eq.${blockedId}),and(owner_id.eq.${blockedId},contact_id.eq.${userId})`);
}

export async function canWizz(senderId: string, recipientId: string) {
  await requireNotBlocked(senderId, recipientId);
  const { data: contact } = await supabaseAdmin.from('contacts').select('favorite').eq('owner_id', recipientId).eq('contact_id', senderId).maybeSingle();
  const { data: privacy } = await supabaseAdmin.from('privacy_settings').select('allow_wizz').eq('user_id', recipientId).maybeSingle();
