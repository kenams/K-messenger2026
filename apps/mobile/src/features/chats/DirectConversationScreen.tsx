...
type HistoryResponse = {
  ok: boolean;
  messages?: Array<{ id: string; senderUserId: string; createdAt: string; algorithm: string }>;
  error?: string;
};
...
export function DirectConversationScreen({ contact, onBack }: { contact: Contact; onBack: () => void }) {
  ...
  const [history, setHistory] = useState<HistoryResponse['messages']>([]);
  ...
  useEffect(() => {
    ...
    let messageHandler: ((message: { id: string; senderUserId: string; createdAt: string; algorithm: string; conversationId: string }) => void) | null = null;
    ...
    void getRealtimeSocket().then(async (client) => {
      ...
      if (active) setHistory(response.messages ?? []);

      messageHandler = (message) => {
        if (message.conversationId !== id) return;
        setHistory((items) => [...(items ?? []), message]);
      };
      client.on('message:new', messageHandler);
      if (active) setNotice('Conversation sécurisée ouverte.');
    }).catch(() => {
...
const styles = StyleSheet.create({
  ...
  envelope: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#dbe9f1', borderRadius: 15, padding: 12, marginBottom: 8 }, envelopeTitle: { color: '#173448', fontWeight: '800' },
