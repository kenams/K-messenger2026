...
- **K-Live**: real-time audio/video broadcast via LiveKit Cloud — "K-Live" button on the Moi screen to go live, app-wide banner for contacts to join as viewer (`live:start/join/stop` sockets). No 1:1 voice/video call exists — this is one-to-many live streaming only. Commit `5d1b155`. (Voice *messages* — hold-to-record notes in DMs/groups — do exist, see below; this line was stale.)
- **Voice notes** (hold-to-record, DM + groups): reuses the private chat media pipeline. Fixed 2026-09-26 (`56da8ac`) — the `media_objects_mime_type_check` constraint from `0010_media_objects.sql` was never widened when `audio/m4a`/`audio/webm` were added to the app's mime allowlist, so every recording succeeded client-side and then failed silently at the very first DB insert of the upload (`media:prepare-upload` ack `{ error: 'REJECTED' }`, no client-visible detail). See `neon/migrations/0031_media_objects_voice_mime.sql` (applied to the live DB) and `apps/mobile/src/lib/media.ts` (`uploadLocalMedia` now retries `media:prepare-upload`/`media:complete-upload` on socket reconnect like `message:send`/`message:delete` already did, and logs a real error code client-side on failure instead of only a generic notice).
...
- **K-Tone**: per-contact custom notification sound + vibration pattern. Client-side-only preference (SecureStore/localStorage, `apps/mobile/src/lib/kTone.ts`), no server/DB change. Wired into direct-message receipt (`DirectConversationScreen.tsx`) and K-Pulse receipt (`MsnContactsScreen.tsx`) via `soundKit.onMessageReceivedFrom`. UI: "🔔 Son personnalisé" in the contact management row. Reuses the 3 existing `assets/sounds/*.wav` as the preset list rather than fabricating new audio assets this session couldn't author reliably.
- **Custom stickers per group**: `neon/migrations/0029_group_stickers.sql` (`group_stickers` table, RLS enabled+forced, membership-scoped, capped at 8/group server-side in `apps/server/src/groupStickerStore.ts`). Reuses the existing chat media pipeline (`media_objects`, purpose `chat`) for storage — no second upload path. Socket handlers `group:sticker-list/add/remove` in `apps/server/src/groupStickerSocket.ts`. Client: sticker tray next to the reaction bar in `GroupEncryptedChat.tsx`, sent as a `type: 'sticker'` `GroupContent` variant through the group's real E2EE (`encryptGroupMessage`) — only pack membership/uploader metadata is server-visible, message content stays end-to-end encrypted.
- **K-Statut**: 24h ephemeral, friends-only, end-to-end encrypted status. `neon/migrations/0030_k_status.sql` (`k_status` + `k_status_keys`, same wrap-per-recipient idiom as `group_keys`). Client `apps/mobile/src/lib/kStatus.ts` mirrors `groupE2ee.ts`'s shape (`postStatus`/`listStatuses`/`openStatus`). Server hard-purges expired rows every 60s (`purgeExpiredKStatuses` in `server.ts`) in addition to the RLS `expires_at > now()` filter, so expiry is a real deletion. UI: `KStatusStrip` on the contacts screen, explicit "Visible par tes contacts uniquement · expire dans Xh" copy, not wired into K-Feed/Moments or any public/algorithmic surface. Same honesty caveat as group keys: a contact added after posting cannot read it retroactively.
- Both migrations above (0029, 0030) exist in the repo but were **not applied to the live Neon database** this session (no `DB_URL` available in this environment). Apply with:
  `DB_URL=<connection string> node scripts/apply-migration.mjs neon/migrations/0029_group_stickers.sql neon/migrations/0030_k_status.sql`

See `docs/VISION_ROADMAP_2027.md` for the status of the rest of the 2027 backlog: #3 (K-Tone), #6 (group stickers) and #13 (K-Statut) are now implemented (see above); the rest were not touched this session — see that document for why, ordered by risk/effort.
