-- K-ssenger Signal prekey claims for direct contacts and current group peers.
-- Authorization remains fail-closed: caller must be either a contact or share a
-- group with the target, and any block in either direction denies the claim.
...
begin
  ...
  if not (
    public.is_contact(v_viewer, v_bundle.user_id)
    or public.shares_group(v_viewer, v_bundle.user_id)
  ) then raise exception 'NOT_AUTHORIZED_PEER'; end if;
